...
class ArticleInfo(BaseModel):
    pui: str = Field(description="PUI (identifier) of the article")
...
def article_text(article: dict[str,str]) -> str:
    skip_fields = ["SOURCE_TITLE", "PUBLICATION_YEAR", "VOLUME", "ISSUE", "DATE_OF_PUBLICATION", "ISSN", "DOI", "FULL TEXT LINK", "EMBASE LINK", "OPEN URL LINK"]
    out_parts = [
        f"{key}: {value}"
...
def generate_prompt(article: dict[str,str]) -> str:
    article_text_content = article_text(article)
    return f"{PROMPT}\n\n{INSTRUCTIONS}\n\n{article_text_content}"


async def extract_study_info(llm: instructor.AsyncInstructor, article: dict[str,str], semaphore: asyncio.Semaphore) -> ArticleInfo:
...
async def main():
    ...
    with open("data/embase_articles_full.json", "r") as f:
        embase_articles = json.load(f)
    print(f"Loaded {len(embase_articles)} articles from embase_articles_full.json")

    # Do the extractions
    llm = instructor.from_openai(openai.AsyncOpenAI())
    selected_articles = random.sample(embase_articles, SAMPLE_SIZE) if SAMPLE_SIZE > 0 else embase_articles
    print(f"Selected {len(selected_articles)} articles for processing")
    semaphore = asyncio.Semaphore(CONCURRENT_REQUESTS)  # Limit concurrent requests
    tasks = [extract_study_info(llm, article, semaphore) for article in selected_articles]
